...
import { ListingCategory, ListingRegion } from '@/lib/data';
import { useAuth } from '@/contexts/AuthContext';
import { useListings } from '@/contexts/ListingContext';
import { useAds } from '@/contexts/AdContext';
import { useNavigation } from '@/contexts/NavigationContext';
import { SearchX, Plus, Rocket, Sparkles } from 'lucide-react';
...
export default function Marketplace() {
  ...
  const { activeAds, incrementImpression, incrementClick } = useAds();
  ...
  // Only show approved listings in marketplace
  ...
  const promotedListings = useMemo(() => {
    return activeAds
      .map(ad => {
        const listing = approvedListings.find(l => l.id === ad.listingId);
        if (listing) {
  ...
  const filteredListings = useMemo(() => {
    let result = approvedListings.filter(listing => !promotedListingIds.has(listing.id));
    ...
    return result;
  }, [approvedListings, promotedListingIds, search, category, region, priceRange, verifiedOnly, sortBy]);
        ...
        <div className="container-wide">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-12">
            ...
            {isAuthenticated && (
              ...
              >
                <Link to="create-listing" className="btn-accent whitespace-nowrap">
          ...
          {promotedListings.length > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {promotedListings.map((listing, index) => listing && (
                  <ListingCard 
                    key={listing.id} 
                    listing={listing} 
                    index={index}
